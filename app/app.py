...
def parse_nlp_input(text):
    ...
    if "tomorrow" in text:
        ...
        if days_until_monday == 0:
            days_until_monday = 7
        result["date"] = (today + timedelta(days=days_until_monday)).strftime(
            "%Y-%m-%d"
        )
...
def quick_add_task():
    ...
    if not is_valid:
        return (
            jsonify({"error": error, "parse_metadata": parsed.get("parse_metadata")}),
            400,
        )
    ...
    response = {**new_task, "parse_metadata": parsed["parse_metadata"]}
...
def check_task_blocked(task_id):
    ...
    blocking_info = [
        {"id": t["id"], "title": t["title"], "completed": t.get("completed", False)}
    ...
    return jsonify(
        {
            ...
            "message": (
                f"Blocked by {len(blocking_tasks)} incomplete dependencies"
                if blocked
                else "Not blocked"
            ),
        }
