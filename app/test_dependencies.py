...
def test_task_dependencies():
    ...
    task_a = requests.post(
        f"{BASE_URL}/tasks", json={"title": "Task A - Foundation", "priority": "high"}
    )
    ...
    task_b = requests.post(
        f"{BASE_URL}/tasks", json={"title": "Task B - Build on A", "priority": "medium"}
    )
    ...
    task_c = requests.post(
        f"{BASE_URL}/tasks", json={"title": "Task C - Final step", "priority": "low"}
    )
    ...
    dep1 = requests.post(
        f"{BASE_URL}/tasks/{task_b_id}/dependencies", json={"dependency_id": task_a_id}
    )
    print_response(f"Add Dependency: B→A", dep1)

    print(f"\n🔹 Step 3: Make C depend on B (C → B)")
    dep2 = requests.post(
        f"{BASE_URL}/tasks/{task_c_id}/dependencies", json={"dependency_id": task_b_id}
    )
    ...
    circular = requests.post(
        f"{BASE_URL}/tasks/{task_a_id}/dependencies", json={"dependency_id": task_c_id}
    )
    ...
    self_dep = requests.post(
        f"{BASE_URL}/tasks/{task_a_id}/dependencies", json={"dependency_id": task_a_id}
    )
    ...
    complete_c = requests.put(f"{BASE_URL}/tasks/{task_c_id}", json={"completed": True})
    ...
    complete_a = requests.put(f"{BASE_URL}/tasks/{task_a_id}", json={"completed": True})
    ...
    complete_b = requests.put(f"{BASE_URL}/tasks/{task_b_id}", json={"completed": True})
    ...
    complete_c2 = requests.put(
        f"{BASE_URL}/tasks/{task_c_id}", json={"completed": True}
    )
    ...
    task_d = requests.post(f"{BASE_URL}/tasks", json={"title": "Task D - Test removal"})
    task_d_id = task_d.json()["id"]

    print(f"\n🔹 Step 14: Add dependency D→A")
    dep_d = requests.post(
        f"{BASE_URL}/tasks/{task_d_id}/dependencies", json={"dependency_id": task_a_id}
    )
