...
def test_focus_sessions():
    ...
    start_response = requests.post(
        f"{BASE_URL}/tasks/{task_id}/focus/start", json={"duration": 25}
    )
    ...
    task_stats_response = requests.get(
        f"{BASE_URL}/api/focus/stats", params={"task_id": task_id}
    )
    ...
    start2_response = requests.post(
        f"{BASE_URL}/tasks/{task_id}/focus/start", json={"duration": 50}
    )
